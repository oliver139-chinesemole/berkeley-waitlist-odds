...
def index_label(index_file: Path) -> str:
    """ "Spring 2026 (term 2262)", from the index's term_id and the meta.json beside it."""
    term_id = json.loads(index_file.read_text()).get("term_id", "?")
    meta = index_file.with_name("meta.json")
    name = json.loads(meta.read_text()).get("term_name") if meta.exists() else None
    return f"{name} (term {term_id})" if name else f"term {term_id}"
...
def main() -> int:
    ...
    previous = json.loads(NAMES_FILE.read_text()).get("_indexes", []) if NAMES_FILE.exists() else []
    indexes = sorted(set(previous) | {index_label(args.index)})
    payload = {
        "_source": url,
        ...
        "_note": (
            ...
            "('Name (CODE)'); one code per subject in every course index listed under '_indexes' (a code once written "
            "is kept, so the file holds for any of those terms' site/data). A name the Guide writes 'X, Y' is stored as 'Y X' and its Guide spelling is kept under 'aliases'. "
