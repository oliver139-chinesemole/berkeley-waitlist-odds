"""``live/latest.json`` on the data branch: the last observation of every active
section, for the site's live counts (docs/DESIGN_A6.md).
...
"Active" means the section is full or has a waitlist; ``--all`` keeps every
section. Only this project's own snapshots feed it; Berkeleytime's history
never does.
"""
...
COLUMNS = ("section_id", "course_key", "subject", "catalog_number", "component", "section_number", "enrolled", "capacity", "waitlist", "waitlist_capacity", "status", "observed_at")
    ...
    *,
    ...
    if state is not None:
        for section_id, r in state.sort_index().iterrows():
            ...
            if only_active and not is_active(enrolled, capacity, waitlist):
                continue
            observed = r["fetched_at"]
            observed_iso = observed.isoformat(timespec="seconds") if hasattr(observed, "isoformat") else str(observed)
            rows.append([str(section_id), r["course_key"], r["subject"], r["catalog_number"], r["component"], r["section_number"], enrolled, capacity, waitlist, wl_capacity, r["status"], observed_iso])
    return {
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Write live/latest.json (last observation of every active section).")
    ...
    p.add_argument("--all", action="store_true", help="keep every section, not only the full or waitlisted ones")
