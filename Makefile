...
FORECAST_TERM ?= 2272
# The finished cycle the site serves (docs/DATA_LOG.md, 2026-09-24: Spring 2026). Only `site-data` writes site/data.
SITE_TERM ?= 2262
...
backfill-analysis:
	python -m analysis.run --backfill-dir $(BACKFILL_DIR) --term-id $(TERM) --out analysis/out --site-dir analysis/out/site_$(TERM) --forecast-term $(FORECAST_TERM) --estimate-level $(ESTIMATE_LEVEL)
...
site-data:
	python -m analysis.export --cohort analysis/out/$(SITE_TERM)/cohort_central.parquet --term-id $(SITE_TERM) --forecast-term $(FORECAST_TERM) --out site/data --flows analysis/out/$(SITE_TERM)/flows.parquet --meta-from site/data/meta.json --estimate-level $(ESTIMATE_LEVEL) $$(test -f $(CATALOG) && echo --catalog $(CATALOG))
