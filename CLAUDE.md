...
- If a named skill or plugin is not loaded, say so once and continue with the same
  discipline by hand.
- Current step: session 10 (2026-09-25, 19:43Z to 20:10Z, Oliver present): every open PR merged into `main` on Oliver's instruction ("you do the merge and resolve the conflicts you noted"), in STATUS section 3's order: #15, #32, #18, #21, #4, #11, #17, #16, #19, #20, #24, #29, #22, #31, #23, #25, #26, #27, #30, #28; `main` is at 55881a7; `pages.yml` deployed 9bae5db (the last site change) at 20:01Z; every page and data file answers 200; live `data/meta.json`: Spring 2026 served for Spring 2027, `estimate_level` course, 2,114 courses; 442 tests on the merged tree; docs/dev/HANDOFF.md session 10 has the merge commits and the six conflict resolutions. Next: Oliver's decisions in HANDOFF session 9 item 2 and his phone for the board; `live/latest.json` appears on the data branch at the next scrape run; `titles.json` reaches the site only when `make site-data` runs with a catalog, so wire the catalog into `analysis.run` before the Spring 2027 switch; Oct 4 term switch. Update this line at the end of every session.
