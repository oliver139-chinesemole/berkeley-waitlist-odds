...
- If a named skill or plugin is not loaded, say so once and continue with the same
  discipline by hand.
- Current step: session 6 closed 2026-09-24 09:00Z; docs/dev/MASTER_PLAN.md leads, docs/dev/HANDOFF.md session 6 has the summary. The collection stack landed on main on 2026-09-24 (09:13Z to 09:19Z, on Oliver's instruction): #8 (scrape.yml 2 per second with 4 in flight after the classes.berkeley.edu slowdown; analysis.yml guard on term 2272) ← #9 (generated priority list, 1,213 live sections a run) ← #10 (catalog titles and instructors) ← #12 (rank lookup, this handoff); and #4 (site v3) ← #11 (the page serves Spring 2026 for Spring 2027, P4 decided). Still open: #4 (site v3) and #11 on it, for Oliver's browser pass and merge; docs/dev/SITE_HANDOFF.md is the site's handoff for the next agent. Next after the merges: note rows for the first run at the new rate and priority_sha, close #7 after a clean 24 h, re-measure CLAIMS rows 7 to 9, Oct 4 term switch (read `selected=`), pages.yml after #11; then search Q1 to Q3, the section board, design U1 to U4. Site v3 (PR #4, worktree `berkeley-waitlist-odds-site`, branch `site-v3`; docs/dev/SITE_V3_PLAN.md section 13, contract docs/DESIGN_A6.md) contains main at a5b36df (merged 2026-09-25) with `site/data` in the v3 format at course level (prereg a7240f4) and `backtest.json` with the cross-term runs; Oliver merges it, then `pages.yml` and a curl of every page. Update this line at the end of every session.
