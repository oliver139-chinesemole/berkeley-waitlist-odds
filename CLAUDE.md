...
- If a named skill or plugin is not loaded, say so once and continue with the same
  discipline by hand.
- Current step: P2 in draft PR #8 (2026-09-23 22:40Z): scrape.yml request policy raised to 2 per second with 4 in flight after the classes.berkeley.edu slowdown (docs/DATA_LOG.md rows dated 2026-09-21, 22 and 23; nine exit-4 runs on Sep 22 to 23, issue #7); docs/dev/MASTER_PLAN.md installed with a verified section 12; analysis.yml's site-data guard restricted to term 2272 (the Fall 2026 own-data cohort showed 209 post-deadline clearings on Sep 23); CLAIMS rows 7 to 9 re-measured and failing until the fix lands. P5 list (top 1,500 sections plus courses with 100+ joins, both cycles; 1,213 live sections a run) in draft PR #9 on branch `p5-priority-list`, stacked on #8. Q7 (course title and instructors parsed into the catalog, DESIGN_A2 section 18) in draft PR #10 on branch `q7-titles`, stacked on #9. Next: Oliver merges #8, #9, then #10; then a note row for the first run at the new rate, 24 h later close #7 and re-measure CLAIMS rows 7 to 9; Oct 4 term switch (read `selected=` in the first Spring run); PR #4 (site v3) still awaits Oliver's review; Spring 2026 source decision open. Update this line at the end of every session.
