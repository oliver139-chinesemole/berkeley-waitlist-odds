...
- If a named skill or plugin is not loaded, say so once and continue with the same
  discipline by hand.
- Current step: site v3 built (2026-09-20, session 5, worktree `berkeley-waitlist-odds-site`, branch `site-v3`, docs/dev/SITE_V3_PLAN.md section 13, contract docs/DESIGN_A6.md): eight pages on shared assets, split site JSON from `analysis/export.py` (`make site-data`), `backtest.json`, `live/latest.json` written by `scrape.yml`, harness tests for every page, Playwright smoke in CI; 353 tests. Waiting on the full-pull install commit (session oliverguo-29 owns STATUS item 1; the chain was restarted 18:23Z with a Cox speed fix), then STATUS item 1b lands the site: rebase, `make site-data --prereg-commit`, merge, `pages.yml`. Then headline results into CLAIMS (same numbers as the Insights page), README hero plot, B5 priority list before Oct 26, Oct 4 term switch, Nov 9 first own-data analysis; GoatCounter, domain, outcome form and the browser pass are Oliver's. Update this line at the end of every session.
