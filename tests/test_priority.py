...
def test_shipped_priority_file_matches_live_site_spellings() -> None:
    """The list installed on 2026-09-23 (docs/DATA_LOG.md priority_list row): built by
    analysis/priority_from_flows.py from both finished cycles' reconstructed joins, in joins
    order. Course keys as classes.berkeley.edu spells them. MATH 1A/1B are gone on purpose:
    Fall 2026 lists calculus as MATH 51 and 52."""
    spec = PrioritySpec.from_file(PRIORITY_FILE)
    assert 990 <= len(spec.patterns) <= 1010
    for key in (
