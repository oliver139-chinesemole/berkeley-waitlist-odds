...
The backtest scores four predictors at each held-out joiner's own horizon (the last automatic waitlist run, the first day of instruction, or a fixed number of days): the position bucket alone, the course-and-bucket curve the site uses, the literal number the site would have shown, and the Cox model. Scores are IPCW Brier, weighted AUC and decile calibration, with a section bootstrap on the gain over the baseline; `tests/test_backtest.py::test_ipcw_toy_example` shows why rows with an unknown outcome are reweighted rather than dropped.
