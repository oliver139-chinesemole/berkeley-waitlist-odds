...
- What an estimate is follows `meta.estimate_level` (`BWO.estimateLevel`). At `dept` (the default): a course's estimate is its department's curve for the bucket, the course's own cases appear as counts ("this course alone: N joiners in S sections", the "This course" column on the course page, the Cases column on Courses), nothing at department level is called pooled (the `pooled` tag and the "Too little data to call" rule apply only to all-course or level pools), the Courses filter hides only all-course pools, the Insights most-and-least list is dropped, and every page prints one sentence on the rule (`BWO.levelNote`) linking the Accuracy page. At `course`, a course with 30 or more cases has its own curve and the department is a labelled fallback.
