...
Next session: (1) after Oliver's note: `make backfill`, `make backfill TERM=2262 TERM_NAME="Spring 2026"`, `make backfill-analysis TERM=2268`, `make backtest TERM=2268`, cross-term B3, re-read the B3 questions, re-measure CLAIMS rows, commit site/data + reports, dispatch pages.yml (pre-registration timestamp); (2) B5 priority list from reconstructed joins before Oct 26; (3) B6 hygiene; (4) 24 h cadence check, close issue #1 after a passing monitor; (5) Oct 4 term switch; (6) Nov 9 first own-data analysis
```
