...
- Request policy since 2026-09-23 (`--min-interval-s 0.5 --max-concurrency 4` in scrape.yml; decision row in docs/DATA_LOG.md): request starts at most 2 per second across all workers, up to 4 pages in flight, so a run's throughput is the smaller of 2 per second and 4 divided by the page latency. The CLI defaults (1 per second, 2 in flight) held until then; from 2026-09-21 14:37Z the site answered in 2 to 5 s with 20 s read timeouts, runs observed 400 to 1,160 of about 1,290 selected sections, and nine runs on Sep 22 to 23 exited 4 and wrote nothing. A run that finishes early sleeps to the next slot like any other.
