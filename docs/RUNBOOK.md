...
**`live/latest.json` on the data branch** (since site v3): every `scrape.yml` run ends by writing the last observation of every full or waitlisted section (`python -m scraper.live`, about 100 KB, 20 KB gzipped) into the same commit as the snapshot, in a step marked `continue-on-error` so it can never fail a collection run. The course page reads it from `raw.githubusercontent.com` and hides the block when the file is absent. If the step starts failing, the Actions log of the step says why; nothing else depends on it.
