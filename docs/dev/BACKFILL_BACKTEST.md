...
- [ ] (after the full Spring 2026 pull; the 20-section probe is too small to train on) If Spring 2026 exists, the real test: `python -m analysis.backtest --cohort analysis/out/2268/cohort_central.parquet --term-id 2268 --split cross_term --train-cohort analysis/out/2262/cohort_central.parquet --which deadline --out reports/backtest_2268/cross_term`. This is the claim the site makes in October: last cycle says something about this one.
- [x] Read the pilot reports (by hand; the `data` plugin is not installed). Answers so far, to be re-read after the full pull:
  - Does `course` beat `bucket`? No: the interval includes zero (temporal) or the two are identical (grouped, where held-out courses fall back to the pool). With 296 sections almost every course cell is pooled anyway; decide on the full pull.
  - Does `cox` beat `bucket`, and is it calibrated? Within a regime (grouped) yes, +0.037; across the Phase 1 to Phase 2 split no, and it is badly calibrated (`days_to_instruction` and the phase dummies extrapolate). The site keeps serving KM curves; Cox is a report, not a product.
  - How bad is `site`? Not worse than the bucket baseline in these tests: the horizon mismatch (curve read at a Phase 1 lead time) over-predicts, and the regime shift makes over-prediction land closer. The B4.1 fix removes the mismatch regardless.
  - Coverage by phase (the replacement for `share_unknown`): at 14 days, phase 1 62%, phase 2 15%, instruction 56%, adjustment 0%. Fall 2026 numbers for the adjustment period are not publishable; the site's floor note handles horizons past a cell's reach.
