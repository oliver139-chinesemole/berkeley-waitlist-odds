...
- Estimates on the site are department-by-position by default (`estimate_level` "dept" in `analysis/export.py` and `meta.json`): the full-pull backtest found course-level curves no better than the position baseline out of time (Brier gain -0.023 [-0.034, -0.011]); `--estimate-level course` is kept for a later term. Cox stays a report.
