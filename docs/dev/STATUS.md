...
| B5 priority list from data | Rank courses by reconstructed joins | Installed 2026-09-23 in a draft PR (branch `p5-priority-list`): N=1500 candidate from both cycles; 1042 live sections in 649 courses per run, 88.1% of joins covered (hand list 47.7%). |
...
3. **B5, installed 2026-09-23 (draft PR, branch `p5-priority-list`)**: `python -m analysis.priority_from_flows ... --top-sections 1500` over both cycles: 999 course patterns, 1042 live sections in 649 courses per run (the hand list fetched 1,047 in 429), selection 1279 with one shard, 88.1% of reconstructed joins covered against 47.7%; `priority_sha` and the numbers in the data-log row. MATH 1A/1B no longer exist (calculus is MATH 51/52). Merge after #8.
