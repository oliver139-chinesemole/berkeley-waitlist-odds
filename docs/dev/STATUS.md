...
| A6 publish | Site, methodology, README | Site v2 with department-level Fall 2026 estimates (PR #5); README hero plot from the backfill. Open: a pass in a real browser; **site v3** in PR #4 (session 5, worktree `berkeley-waitlist-odds-site`, branch `site-v3`; docs/dev/SITE_V3_PLAN.md section 13): eight pages on shared assets, split JSON re-exported from the installed cohort at department level with prereg a7240f4, `backtest.json`, `live/latest.json` writer, harness tests for every page, Playwright smoke in CI; Oliver merges it. |
