...
| A6 publish | Site, methodology, README | Site v2 with course-level Fall 2026 estimates (PR #6); README hero plot from the backfill. Open: a pass in a real browser; site v3 in PR #4 (another session); the Spring 2026 source decision. **Site v3** in PR #4 (session 5, worktree `berkeley-waitlist-odds-site`, branch `site-v3`; docs/dev/SITE_V3_PLAN.md section 13): eight pages on shared assets, split JSON re-exported from the installed cohort at course level with prereg a7240f4, `backtest.json` with the cross-term runs, `live/latest.json` writer, harness tests for every page, Playwright smoke in CI; Oliver merges it. |
...
| B5 priority list from data | Rank courses by reconstructed joins | Tool done; both cycles' flows exist now (`backfill/2268`, `backfill/2262`); install before Oct 26 (section 4, item 3). |
...
0. **Merge site v3** (PR #4, ready for review; the branch already contains `main` at e89fe9d and `site/data` re-exported in the v3 format from the installed cohort): review, merge, `gh workflow run pages.yml`, then curl every page for 200 (`for p in index courses course insights accuracy methodology about 404; do curl -sS -o /dev/null -w "$p %{http_code}\n" https://oliver139-chinesemole.github.io/berkeley-waitlist-odds/$p.html; done`) and `data/index.json`, `data/pooled.json`, `data/insights.json`, `data/backtest.json`; download the `site-smoke` artifact to see the pages. The weekly `analysis.yml` stays compatible: its `analysis.run --site-dir analysis/out/site_candidate` already writes the v3 layout (run.py calls the v3 exporter; `tests/test_site.py::test_meta_from_the_analysis_run_renders_as_numbers` renders that output), so the guard's copy over `site/data` at 10 own-data clearings needs no change; the weekly run writes no `backtest.json`, so after the switch the Accuracy page says the backtest is not published until `make backtest TERM=2272` and `python -m analysis.export_backtest --reports reports/backtest_2272 --term-id 2272 --term-name "Spring 2027" --out site/data/backtest.json` are run by hand. If the cross-term result (item 1) changes the level, `make site-data` (`ESTIMATE_LEVEL=...`) rewrites the v3 files; a `bucket` level would need adding to `analysis/export.py` and the pages' copy.
...
- `estimate_level` (`analysis/export.py`, `meta.json`, `--estimate-level`, Makefile `ESTIMATE_LEVEL`) is what a course's estimate is. Within Fall 2026 the course curve lost to the position bucket out of time (the Phase 1 to Phase 2 speed-up), so the site went to `dept` for a few hours on 2026-09-20; the cross-term backtest (Spring 2026 to Fall 2026) showed course identity transfers, so the default is `course` (own curve at 30+ cases, department then all courses as labelled fallbacks). Cox stays a report.
