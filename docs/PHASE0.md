...
- Listing: `GET /search/class?f[0]=term:<facet-id>&page=<n>` renders 18 sections per page (Fall 2026 facet `8588`, 6,131 sections, 341 pages), but **`/search/` is disallowed by robots.txt** (see below), so the scraper does not use it. The section universe comes from the site itself: `/rss.xml` (allowed; the 10 newest nodes with their ids and titles like `2026 Fall AEROENG 10 001 LEC 001`) and `/node/<id>` (allowed; the section page for a node id, or 404). The scraper keeps a node-id watermark in `catalog/site.json` and enumerates new ids each run, so newly published sections (Spring 2027 on Oct 4) are found without any third party. Berkeleytime's `GetCatalog` was the first replacement for the listing but is unreachable from GitHub-hosted runners (Cloudflare 403 for every client, diagnosed 2026-09-19), so it is not used for discovery. Self-study components (IND, GRP, FLD, TUT, ...) are excluded: they never carry a waitlist.
...
- Spring 2027 is not on the site yet. The registrar publishes the Spring 2027 Schedule of Classes on **Oct 4, 2026**; its section pages appear as new nodes, the scraper enumerates them over the following runs, and the SIS term id is confirmed from the pages' `data-term` attribute.
